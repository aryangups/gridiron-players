...
class Settings:
    ...
    enable_player_stats: bool = _bool("ENABLE_PLAYER_STATS", True)
    request_delay_seconds: float = _float("REQUEST_DELAY_SECONDS", 0.5)
    stats_request_delay_seconds: float = _float("STATS_REQUEST_DELAY_SECONDS", 0.05)
    max_news_per_player: int = _int("MAX_NEWS_PER_PLAYER", 10)
    max_teams_initial_run: int = _int("MAX_TEAMS_INITIAL_RUN", 200)
    max_stats_players: int = _int("MAX_STATS_PLAYERS", 0)
    full_run: bool = _bool("FULL_RUN", True)
    espn_season: int = _int("ESPN_SEASON", 2025)
    espn_fbs_group_id: int = _int("ESPN_FBS_GROUP_ID", 80)
    espn_cfb_poll_lookback_days: int = _int("ESPN_CFB_POLL_LOOKBACK_DAYS", 1)
    espn_cfb_poll_lookahead_days: int = _int("ESPN_CFB_POLL_LOOKAHEAD_DAYS", 1)
    espn_cfb_max_games: int = _int("ESPN_CFB_MAX_GAMES", 0)
